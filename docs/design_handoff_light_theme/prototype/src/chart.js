...
(function (global) {
  ...
  // ── Colour helpers (theme-aware: re-read CSS vars on each render) ───────
  const C = {};

  /** Read theme colors from CSS custom properties and update C in place. */
  function refreshColors() {
    ...
    Object.assign(C, {
      bg:         v("--chart-bg",     "#09131f"),
      bgDay:      v("--chart-day",    "rgba(28,74,150,0.14)"),
      tempFill:   isLight
        ? ["rgba(234,124,28,0.06)", "rgba(234,124,28,0.34)"]
        : ["rgba(255,160,30,0.05)", "rgba(255,140,20,0.38)"],
      tempLine:   v("--accent-warm",  "#ffa820"),
      feelsLine:  v("--accent-cyan",  "rgba(60,220,255,0.75)"),
      precipLow:  v("--accent-green", "#30d67a"),
      precipMid:  v("--accent-warm",  "#f0b030"),
      precipHi:   v("--accent-red",   "#ff5055"),
      pressLine:  v("--chart-press",  "rgba(255,255,255,0.32)"),
      nowLine:    v("--chart-now",    "rgba(255,255,255,0.65)"),
      grid:       v("--chart-grid",   "rgba(255,255,255,0.06)"),
      dayDiv:     v("--card-border",  "rgba(255,255,255,0.10)"),
      text:       v("--text",         "#e4eaf4"),
      textMuted:  v("--text-fade",    "#5a7a9a"),
      cloud:      v("--chart-cloud",  "rgba(210,230,255,1)"),
      axis:       v("--chart-axis",   "rgba(255,255,255,0.28)"),
      windArrow:  isLight ? "rgba(20,40,80,0.55)" : "rgba(255,255,255,0.65)",
    });
  }
  refreshColors();

  // ── Theme-derived chart helpers ─────────────────────────────────────────
  function isLight() { return document.documentElement.getAttribute("data-theme") === "light"; }

  /** Cloud puff fill: light=grey-blue alpha, dark=white-blue alpha. */
  function cloudFillFor(cc) {
    return isLight()
      ? `rgba(110,135,170,${cc * 0.18})`
      : `rgba(210,230,255,${cc * 0.22})`;
  }
  /** Halo behind temp labels — contrasts with chart bg. */
  function labelStroke() {
    return isLight() ? "rgba(255,255,255,0.85)" : "rgba(9,19,31,0.7)";
  }
  /** Subtle strip behind the day-axis text. */
  function dayAxisStrip() {
    return isLight() ? "rgba(20,40,80,0.05)" : "rgba(0,0,0,0.18)";
  }
  ...
  class WeatherChart {
    ...
    // ── Render entry point ─────────────────────────────────────────────────
    render() {
      refreshColors();   // pick up current theme on every paint
      const { ctx } = this;
    ...
    _drawCloudCover() {
      ...
      for (let i = 0; i < hours.length; i++) {
        ...
        const ry = 14 * cc;
        // Build cloud fill from theme cloud color + cc alpha
        ctx.fillStyle = cloudFillFor(cc);
        ctx.beginPath();
    ...
    _drawTempLabels() {
      ...
      for (let i = 1; i < hours.length - 1; i++) {
        ...
        // Halo around temp labels: contrasts with the chart bg in both themes
    ...
    _drawDayAxis() {
      ...
      // Subtle background strip for the day axis (lighter or darker than chart bg)
      ctx.fillStyle = dayAxisStrip();
